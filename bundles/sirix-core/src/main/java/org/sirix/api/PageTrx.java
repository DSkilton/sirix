...
public interface PageTrx extends PageReadOnlyTrx {
    ...
    /**
     * Create fresh key/value (value must be a record) and prepare
     * key/value-tuple for modifications (CoW). The record might be a node, in
     * this case the key is the node.
     *
     * @param key optional key associated with the record to add (otherwise the
     ...
     * @throws NullPointerException if {@code record} or {@code page} is
     * {@code null}
     */
     ...
     * entry for upcoming modification. The key of the entry might be the node
     * key and the value the node itself.
     *
     * @param key key of the entry to be modified
     * @param indexType the index type
     * @param index the index number
     * @return instance of the class implementing the {@link DataRecord}
     * instance
     * @throws SirixIOException if an I/O-error occurs
     ...
     * @throws SirixIOException if something odd happens while storing the new
     * key
     * @throws NullPointerException if {@code name} or {@code kind} is
     * {@code null}
     */
     ...
     *
     * @param restore determines if this page write trx must restore a previous
     * failed trx
