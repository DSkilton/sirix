...
public interface NodeCursor extends AutoCloseable {
     ...
     * with{Moved#get()}. In case the node does not exist
     ...
     * of the current node.
     *
     * @return {@link Moved} instance if the next following node is selected,
     * {@code NotMoved} instance otherwise
     */
    ...
    /**
     * Move cursor to left sibling node of the currently selected node. Check
     * the postcondition with {@code Moved#hasMoved()} or get the current cursor
     * instance with{Moved#get()}. In case the node does not exist
     * {@code Moved#hasMoved()} returns false and the cursor has not been moved.
     *
     * @return {@link Moved} instance if the attribute node is selected,
    ...
    /**
     * Move cursor to right sibling node of the currently selected node. Check
     * the postcondition with {@code Moved#hasMoved()} or get the current cursor
     * instance with{Moved#get()}. In case the node does not exist
     * {@code Moved#hasMoved()} returns false and the cursor has not been moved.
     *
    ...
    /**
     * Move cursor to the previous node of the currently selected node in
     * document order. Check the postcondition with {@code Moved#hasMoved()} or
     * get the current cursor instance with{Moved#get()}. In case the node does
     * not exist {@code Moved#hasMoved()} returns false and the cursor has not
     * been moved.
     *
     ...
     * current cursor instance with{Moved#get()}. In case the node does not
     * exist {@code Moved#hasMoved()} returns false and the cursor has not been
     * moved.
     *
    ...
    /**
     * Determines if the node located at the current cursor position has a
     * parent.
     *
    ...
    /**
     * Get unique node key, that is the unique ID of the currently selected
     * node.
     *
    ...
    /**
     * Get the kind/type of node of the last child of the currently selected
     * node.
     *
