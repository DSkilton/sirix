...
public interface PageReadOnlyTrx extends AutoCloseable {
     ...
     * @return an {@link Optional} reference usually containing the node
     * reference
     ...
     *
     * @param indexLogKey it has the key {@code key} of key/value page to get
     * the record from, the index number or {@code -1}, if it's a regular record
     * page to lookup and the kind of page to lookup
     * @param <K> Key
     * @param <V> Value
     * @param <T> Instance of class, implementing the {@link KeyValuePage}
     * interface
     ...
     *
     * @param revisionRoot {@link RevisionRootPage} for which to get the
     * {@link NamePage}
     ...
     * @throws SirixIOException if an I/O error occur@Nonnull RevisionRootPage
     * revisionRoots
     */
    ...
    /**
     * Get the {@link PathSummaryPage} associated with the current revision
     * root.
     *
     * @param revisionRoot {@link RevisionRootPage} for which to get the
     ...
     *
     * @param startReference the start reference (for instance to the indirect
     * tree or the root-node of a BPlusTree)
