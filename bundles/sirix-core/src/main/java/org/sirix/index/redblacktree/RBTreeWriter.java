...
public final class RBTreeWriter<K extends Comparable<? super K>, V extends References>
     ...
     * @param move determines if AVLNode cursor must be moved to document
     * root/root node or not
    ...
    private long getNewNodeKey(final RevisionRootPage root) {
        ...
        return switch (rbTreeReader.indexType) {
            case PATH ->
                pageTrx.getPathPage(root).getMaxNodeKey(rbTreeReader.index) + 1;
            case CAS ->
                pageTrx.getCASPage(root).getMaxNodeKey(rbTreeReader.index) + 1;
            case NAME ->
                pageTrx.getNamePage(root).getMaxNodeKey(rbTreeReader.index) + 1;
            case PATH_SUMMARY ->
                pageTrx.getPathSummaryPage(root).getMaxNodeKey(rbTreeReader.index) + 1;
            default ->
    ...
    /**
     * Remove a node key from the value, or remove the whole node, if no keys
     * are stored anymore.
     *
