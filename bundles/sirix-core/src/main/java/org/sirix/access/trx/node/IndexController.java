...
public interface IndexController<R extends NodeReadOnlyTrx & NodeCursor, W extends NodeTrx & NodeCursor> {
     ...
     *
     * @param type type of index to lookup
     * @param resourceManager the {@link ResourceManager} this index controller
     * is bound to
     * @return {@code true} if an index of the specified type exists,
     * {@code false} otherwise
     * @throws SirixIOException if an I/O exception occurs while deserializing
     * the index configuration for the specified {@code revision}
     */
     ...
     * @param pathNodeKey the path node key of the node (might also be the path
     * node key of the parent node)
     ...
     * @throws SirixIOException if an I/O exception during index creation
     * occured
     */
     ...
     *
     * @param in the {@link InputStream} from which to deserialize the XML
     * fragment
