...
public final class JsonRecordSerializerTest {
    ...
    public void serializeArray() {
        ...
        try (final var database = Databases.openJsonDatabase(JsonTestHelper.PATHS.PATH1.getFile())) {
            ...
            try (final var resmgr = database.openResourceManager(JsonTestHelper.RESOURCE); final var wtx = resmgr.beginNodeTrx()) {
                final var json = """
                [{},"bla",{"foo":"bar"},null,[]]
            """.strip();
    ...
    public void serializeObject() {
        ...
        try (final var database = Databases.openJsonDatabase(JsonTestHelper.PATHS.PATH1.getFile()); final var resmgr = database.openResourceManager(JsonTestHelper.RESOURCE)) {
            final var stringWriter = new StringWriter();
            final var jsonRecordSerializer = new JsonRecordSerializer.Builder(resmgr, 3, stringWriter).build();
            jsonRecordSerializer.call();
